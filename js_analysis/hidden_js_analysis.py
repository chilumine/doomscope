...
def extract_js_from_file(file_path):
    ...
    js_regex = r"https?://[^\s'\"<>]+\.js(\?[^\s'\"<>]*)?"

    with open(file_path, "r", encoding="utf-8", errors="ignore") as f:
        for line in f:
            found = re.findall(js_regex, line)
            if found:
                # found = ONLY the query part, so we need full match using search
                matches = re.finditer(js_regex, line)
                for m in matches:
...
def scan_domain(domain):
    base = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "..", "directory_search", "results")
    )

    folder = os.path.join(base, underscore(domain))
    ...
    # اقرأ كل ملفات JSON/TXT
    for file in os.listdir(folder):
        if file.endswith(".json") or file.endswith(".txt"):
            path = os.path.join(folder, file)
            all_js.update(extract_js_from_file(path))
