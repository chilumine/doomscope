...
def load_js_urls(domain):
    base_dir = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "..", "archived_contents")
    )

    filename = f"archived_js_files_{underscore(domain)}.txt"
    path = os.path.join(base_dir, filename)

    if not os.path.isfile(path):
        return None, f"JS file not found: {path}"
...
def save_results(domain, data):
    base_dir = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "archived_js")
    )
...
def scan_domain(domain):
    js_urls, error = load_js_urls(domain)
    if error:
        return {"error": error}
    ...
    saved_path = save_results(domain, result)
